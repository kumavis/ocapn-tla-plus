...
(* ref / wire (here: forwards via vats[vatA].refs[2] =                    *)
(* RemoteTarget(vatC, 2), sitting on channels[vatA][vatC] with refId=2)   *)
(* is unrelated and must NOT trigger a spurious embargo.                  *)
(*                                                                         *)
(* Pre-state:                                                              *)
...
(*   vats[vatA].refs[1] = RemotePromise(vatB, 1, ResNone, embargo=FALSE)   *)
...
(*   vats[vatA].refs[1].fresh = TRUE (vatA has never pipelined through    *)
(*   ref 1; the unrelated forward on channels[vatA][vatC] went through    *)
(*   ref 2, which clears vats[vatA].refs[2].fresh -- a per-ref bit --     *)
(*   not vats[vatA].refs[1].fresh).  Therefore the fast path fires,       *)
(*   embargo stays FALSE on vats[vatA].refs[1], and no flush probe is     *)
(*   emitted.                                                             *)
(*                                                                         *)
